...
3. **Check for at least 2 commits.** Run `git rev-list --count HEAD -- .manuscript/ 2>/dev/null`. If only 1: "Nothing to undo. This is your first save."
...
5. **Read the last commit message** to describe what will be undone. Run: `git log -1 --format="%s" .manuscript/`

6. **Show the confirmation prompt:**

   ```
   This will revert to your previous save. You'll lose:
   - Changes from "{last commit message}" ({summary of what changed})
...
7. **If the writer says "yes":**
   - Run: `git revert HEAD --no-edit` to create a new commit that undoes the last save (safe, preserves history)
...
- **Use `git revert` instead of `git reset`** to preserve history. The writer can always undo the undo.
...
- **Developer mode** (`developer_mode: true`): Show git revert output, the hash of the revert commit, and the hash of the commit that was reverted.
