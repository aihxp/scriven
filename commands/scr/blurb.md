...
2. `data/CONSTRAINTS.json` -- verify `blurb` is available for this work type (available: prose, script, visual, poetry, interactive, sacred; hidden: academic, speech_song). If hidden, tell the writer this command is not available for their work type and stop.
