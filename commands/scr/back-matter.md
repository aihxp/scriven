...
|---------|-----------|------|-------------|
| Concordance | `concordance` | GENERATE | Reference to CONCORDANCE.md (generated by `/scr:concordance`). If CONCORDANCE.md exists, include it as an appendix. If not, suggest running `/scr:concordance` first and provide a scaffold structure for manual creation. |
...
| Chronology Appendix | `chronology-appendix` | GENERATE | Timeline from CHRONOLOGY.md. If CHRONOLOGY.md exists, format as a chronological table. If not, suggest running `/scr:chronology` first. |
...
<!-- If not, run /scr:concordance to generate one from the manuscript. -->
